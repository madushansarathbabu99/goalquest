...
export default function EditGoalModal({ goal, onClose, onSave, currentUserId }) {
  ...
  const [taggedFriendId, setTaggedFriendId] = useState(goal.tagged_friend_id || null)
      ...
      <div className="modal">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="field">
            ...
            <input type="datetime-local" value={deadline} onChange={e => setDeadline(e.target.value)} required disabled={goal.completed} />
          </div>
          <div className="field">
            <label>Points on Completion</label>
            <div className="points-select">
              {pointOptions.map(p => (
                <button type="button" key={p} className={`points-opt ${points === p ? 'active' : ''}`} onClick={() => setPoints(p)} disabled={goal.completed}>{p}</button>
              ))}
            </div>
          </div>
          <div className="field">
            ...
            <div className="gift-input-wrap">
              ...
              <input type="text" placeholder="e.g. New book, movie night, treat yourself…" value={gift} onChange={e => setGift(e.target.value)} className="gift-input" />
            </div>
